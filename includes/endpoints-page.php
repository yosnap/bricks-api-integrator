    ...
    function render_api_endpoints_page() {
        ...
        jQuery(document).ready(function($) {
            ...
            function resetForm() {
                ...
                $('#save-endpoint-btn').text('Guardar Endpoint');
            ...
            $('#endpoint-form').submit(function(e){
                ...
                $.post(ajaxurl, data, function(resp){
                    if (resp.success) {
                        ...
                        renderTable();
                        resetForm();
                        showMessage('¡Endpoint guardado correctamente!', 'success');
                    } else {
                        ...
                        if (resp.success && resp.data && resp.data.tags && resp.data.tags.length) {
                            ...
                            tags.forEach((tagObj, idx) => {
                                ...
                                html += `<div style="display:flex;align-items:center;gap:10px;background:#f8f9fa;padding:8px 12px;border-radius:5px;">
                                    ...
                                    <button type="button" class="button button-small" style="margin-left:10px;" onclick="navigator.clipboard.writeText(String(${JSON.stringify(tagObj.example)}));this.innerText='¡Copiado!';setTimeout(()=>{this.innerText='Copiar valor';},1000);">Copiar valor</button>
                                </div>`;
                            });
            ...
            // --- Cancelar edición ---
            $('#cancel-edit-btn').click(function(){ resetForm(); });
            // --- Eliminar endpoint ---
            ...
            $('#view-tags-btn').click(function(){
                ...
                tags.forEach(tag => { html += `<li><code>${tag}</code></li>`; });
                html += '</ul></div>';
                showTestResult(html);
            });
...
add_action('wp_ajax_test_api_endpoint', function() {
    ...
    if ($code >= 200 && $code < 300) {
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            ...
            // Mostrar campos detectados si es array asociativo
            if (is_array($data)) {
...
add_action('wp_ajax_generate_tags_for_endpoint', function() {
    ...
    if ($url && $name) {
        ...
        if (!is_wp_error($response)) {
            ...
            // Generar tags recursivos a partir del ejemplo
            $tags = function_exists('bricks_api_extract_tags_recursive')
                ? bricks_api_extract_tags_recursive($example, 'snap_' . $slug)
                : [];
        }
...
add_action('wp_ajax_get_saved_tags_for_endpoint', function() {
    ...
    if ($name) {
        ...
        if (isset($tags_data[$slug])) {
            ...
            $example_json = json_encode($example, JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);
            // Si los tags guardados son solo strings, reconstruir la estructura avanzada
            if ($tags && is_string($tags[0])) {
                // Volver a extraer la estructura avanzada de tags
                $tags = function_exists('bricks_api_extract_tags_recursive') ? bricks_api_extract_tags_recursive($example, 'snap_' . $slug) : [];
            }
            wp_send_json_success([
                'tags' => $tags,
                'example' => $example,
